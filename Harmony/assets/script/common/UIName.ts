/**
 * 界面名字
 */
export default class UIName {
    public static readonly LOGIN : string = 'LOGIN';
    public static readonly MSGBOX : string = 'MSGBOX';
    public static readonly ROCKVIEW : string = 'ROCKVIEW';
    public static readonly TESTVIEW : string = 'TESTVIEW';
    public static readonly PLAYER_PGB_VIEW : string = 'PLAYER_PGB_VIEW';
    public static readonly ADVERTISEMENT_VIEW : string = 'ADVERTISEMENT_VIEW'; //广告界面
    public static readonly SELECTSKILL_VIEW : string = 'SELECTSKILL_VIEW'; //选择技能界面
    public static readonly SELECTSKILL_ITEM_VIEW : string = 'SELECTSKILL_ITEM_VIEW'; //选择技能界面

    
}