...
--	file:f
--
function output_table(t, f, tabs, comma, w_tbl,isfrist,filename)
	...
	for _, k in ipairs(k_tbl) do
		...
		if tv=="table" then
			f:write(tabs, k, ": ")
			output_table(v, f, tabs .. "	", true,nil,false)
...
function output_table_json(t, f, tabs, comma, w_tbl,isend,filename)
	if not (type(t)=="table") then
		print("Invalid table to output!")
		return
	end
	...
	for _, k in ipairs(k_tbl) do
		...
		if tv=="table" then
			...
			f:write(tabs, k )
			output_table_json(v, f, tabs .. "", noarray,nil,_==#k_tbl,nil)
		elseif tv=="string" then
			if v:sub(1,1)=="\"" and v:sub(-1)=="\"" then
